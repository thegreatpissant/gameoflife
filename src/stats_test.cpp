...
protected:
  ...
  void SetUp() override {
    ...
    cout_redirector_ptr =
        std::make_unique<CoutRedirector>(captured_cout_stream.rdbuf());
  }
  ...
  std::string getCapturedOutput() { return captured_cout_stream.str(); }
};
...
TEST_F(StatsTest, Print_NoStats) {
  ...
  // Based on the assumed print(), if counters is empty, it prints
  // "Statistics:\n" and returns.
...
// Test case: Calling start() multiple times for the same counter (last call
// wins)
...
// Test case: Calling stop() multiple times for the same counter (last call
// wins)
...
TEST_F(StatsTest, StartWithoutStop) {
  ...
  // This test highlights that the simple print() can produce confusing output
  // for this case. A production-ready print() might show "(not stopped)" or
  // similar.
}
